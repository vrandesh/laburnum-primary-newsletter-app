...
ARCHIVE_URL = "https://www.laburnumps.vic.edu.au/enews/this_year"
DEFAULT_OUTPUT = "laburnum_enews_this_year.json"
...
def scrape_newsletters(archive_url: str, verify_ssl: bool) -> dict[str, Any]:
    archive_html = fetch_html(archive_url, verify_ssl=verify_ssl)
    archive_cards = parse_archive_cards(archive_html, archive_url)
    ...
    for card in archive_cards:
        ...
        newsletters.append({**card, **detail_data})
...
def load_or_fetch_data(
    ...
    data = scrape_newsletters(archive_url, verify_ssl=verify_ssl)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch Laburnum Primary School eNews newsletters from the current-year archive."
    )
    parser.add_argument("--archive-url", default=ARCHIVE_URL, help="Archive page to scrape.")
    parser.add_argument(
        "--output",
